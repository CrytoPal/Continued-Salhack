...
public final class AutoTrap extends Module {
    private static final List<Vec3d> offsetsDefault = Arrays.asList(
    ...
    private static final List<Vec3d>  offsetsTall = new ArrayList<>(offsetsDefault);
    static {
        offsetsTall.add(new Vec3d(0.0, 4.0, 0.0));
    }
    ...
    public AutoTrap() {
        super("AutoTrap", new String[]{ "AutoTrap" }, "Traps enemies in obsidian", 0, 0x24DB43, Module.ModuleType.COMBAT);
    }
    ...
    public String getMetaData() {
        if (EChests.getValue()) return "Ender Chests";
        return "Obsidian";
    }
    ...
    public void onEnable() {
        ...
        if (findObiInHotbar() == -1) {
            sendMessage(String.format("[AutoTrap] You do not have any %s in your hotbar!", Formatting.LIGHT_PURPLE + (EChests.getValue() ? "Ender Chests" : "Obsidian") + Formatting.RESET));
            toggle(true);
        }
    }
    ...
    public void onDisable() {
        super.onDisable();
        if (mc.player != null) {
            if (lastHotbarSlot != playerHotbarSlot && playerHotbarSlot != -1) mc.player.getInventory().selectedSlot = playerHotbarSlot;
            if (isSneaking) {
            ...
            playerHotbarSlot = -1;
            lastHotbarSlot = -1;
            if (announceUsage.getValue()) sendMessage("[AutoTrap] Disabled!");
        }
    }

    @EventHandler
    public void onPlayerUpdate(PlayerMotionUpdate event){
        if (event.getEra() != EventEra.PRE || mc.world == null) return;
        PlayerEntity closestTarget = PlayerUtil.findClosestTarget();
        if (mc.player != null) {
            if (closestTarget == null) {
                if (firstRun) {
                    firstRun = false;
                    if (announceUsage.getValue()) sendMessage("[AutoTrap] Enabled, waiting for target.");
                }
                return;
            }
            if (firstRun) {
                ...
                if (announceUsage.getValue()) sendMessage("[AutoTrap] Enabled, target: " + lastTickTargetName);
            } else if (!lastTickTargetName.equals(closestTarget.getName().getString())) {
                lastTickTargetName = closestTarget.getName().getString();
                offsetStep = 0;
                if (announceUsage.getValue()) sendMessage("[AutoTrap] New target: " + lastTickTargetName);
            }
            ...
            switch (Mode.getValue()) {
                case Full -> placeTargets.addAll(offsetsDefault);
                case Tall -> placeTargets.addAll(offsetsTall);
                default -> {}
            }
            int blocksPlaced = 0;
            while (blocksPlaced < blockPerTick.getValue()) {
                ...
                boolean shouldTryToPlace = mc.world.getBlockState(targetPos).isReplaceable();
                ...
                if (shouldTryToPlace && placeBlock(targetPos)) ++blocksPlaced;
                ++offsetStep;
            }
            if (blocksPlaced > 0) {
    ...
    private boolean placeBlock(final BlockPos pos) {
        ...
        if (!BlockInteractionHelper.checkForNeighbours(pos)) return false;
    ...
    private int findObiInHotbar() {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (stack != ItemStack.EMPTY && stack.getItem() instanceof BlockItem) {
                ...
                if (EChests.getValue() && block instanceof EnderChestBlock) return i;
                else if (block == Blocks.OBSIDIAN) return i;
            }
        }
        return -1;
    }
